...
const PropostasTab = ({ dadosCliente, dadosCard }: any) => {
  ...
  const [idPropostaSelecionada, setIdPropostaSelecionada] = useState();
  const [idKitProposta, setIdKitProposta] = useState<any>();
  ...
  const buscaDadosKitProposta = async (idKitProposta: any) => {
    await listarComponentesKit(idKitProposta).then((res: any) => {
      setKitProposta(res);
    });
  };
  ...
  const handleOpenDadosProposta = (idProposta: any) => {
    setViewDados(!viewDados);
    ...
    <div className="flex flex-col">
      ...
      {view === "Propostas" ? (
        !viewDados ? (
          <>
            ...
            {filteredPropostas.map((proposta, index) => {
              ...
              return (
                <div
                  onClick={() => {
                    handleOpenDadosProposta(proposta.id_proposta);
                    buscaDadosKitProposta(proposta.kit_proposta);
                  }}
                  key={index}
                          ...
                          <>
                            <div key={statusIndex} className="grid gap-2">
                              ...
                              <p className="flex justify-end mr-2">
                                {format(
                                  new Date(proposta.data_proposta),
                                  "dd/MM/yyyy"
                                )}
                              </p>
          ...
          <div>
            <p>Conteudo da proposta {idPropostaSelecionada} </p>
            {/* {kitProposta.componentesKit.map((kit)=>{
              console.log(kit);
             return(
